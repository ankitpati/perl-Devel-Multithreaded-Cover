...
package Devel::Multithreaded::Cover;
...
use Devel::Multithreaded::Cover::DB;
use Devel::Multithreaded::Cover::DB::Digests;
use Devel::Multithreaded::Cover::Inc;

BEGIN { $VERSION //= $Devel::Multithreaded::Cover::Inc::VERSION }
...
my $Self_cover_run = 0;                  # Covering Devel::Multithreaded::Cover now
...
use vars '$File',                        # Last filename we saw.  (localised)
         ...
         '$Self_cover';                  # Coverage of Devel::Multithreaded::Cover

BEGIN {
    ($File, $Line, $Collect) = ("", 0, 1);
    $Silent = ($ENV{HARNESS_PERL_SWITCHES} || "") =~ /Devel::Multithreaded::Cover/ ||
              ($ENV{PERL5OPT}              || "") =~ /Devel::Multithreaded::Cover/;
    *OUT = $ENV{DEVEL_MULTITHREADED_COVER_DEBUG} ? *STDERR : *STDOUT;

    if ($] < 5.010000 && !$ENV{DEVEL_MULTITHREADED_COVER_UNSUPPORTED}) {
...
Devel::Multithreaded::Cover $VERSION is not supported on perl $].  The last version of
Devel::Multithreaded::Cover which was supported was version $v.  This version may not work.
...
If you are actually using this version of Devel::Multithreaded::Cover with perl $], please let
me know.  I don't want to know if you are just testing Devel::Multithreaded::Cover, only if you
...
In order to use this version of Devel::Multithreaded::Cover with perl $] you must set the
environment variable \$DEVEL_MULTITHREADED_COVER_UNSUPPORTED
...
EOM
    ...
    if ($^X =~ /(apache2|httpd)$/) {
        # mod_perl < 2.0.8
        @Inc = @Devel::Multithreaded::Cover::Inc::Inc;
    } else {
    ...
    @Ignore = ("/Devel/Multithreaded/Cover[./]") unless $Self_cover = $ENV{DEVEL_MULTITHREADED_COVER_SELF};
...
Unfortunately, Devel::Multithreaded::Cover does not yet work with threads.  I have done
...
sub import {
    ...
    my $options = ($ENV{DEVEL_MULTITHREADED_COVER_OPTIONS} || "") =~ /(.*)/ ? $1 : "";
    ...
    bootstrap Devel::Multithreaded::Cover $VERSION;
    ...
    $DB = $1 if abs_path($DB) =~ /(.*)/;
    Devel::Multithreaded::Cover::DB->delete($DB) unless $Merge;

    %Files = ();  # start gathering file information from scratch

    for my $c (Devel::Multithreaded::Cover::DB->new->criteria) {
        my $func = "coverage_$c";
        no strict "refs";
        $Criteria{$c} = $func->();
    }
...
sub populate_run {
    ...
    if (-e $mymeta) {
        eval {
            require Devel::Multithreaded::Cover::DB::IO::JSON;
            my $io   = Devel::Multithreaded::Cover::DB::IO::JSON->new;
...
sub normalised_file {
    ...
    $Digests ||= Devel::Multithreaded::Cover::DB::Digests->new(db => $DB);
...
sub use_file {
    ...
    print STDERR __PACKAGE__ . qq(: Can't find file "$file" (@_): ignored.\n)
        unless $Files{$file} || $Silent
                             || $file =~ $Devel::Multithreaded::Cover::DB::Ignore_filenames;
...
Devel::Multithreaded::Cover: Oops, it looks like something went wrong writing the coverage.
...
sub _report {
    ...
    die "Devel::Multithreaded::Cover::import() not run: " .
        "did you require instead of use Devel::Multithreaded::Cover?\n"
        unless defined $Dir;
    ...
    $Structure      = Devel::Multithreaded::Cover::DB::Structure->new(
        base        => $DB,
    ...
    my $cover = Devel::Multithreaded::Cover::DB->new(
        base        => $DB,
        runs        => { $run => \%Run },
        structure   => $Structure,
        loose_perms => $Loose_perms,
...
Devel::Multithreaded::Cover - Code coverage metrics for Perl
...
The F<cover> program can be used to generate coverage reports.  Devel::Multithreaded::Cover
...
Some annotation modules are shipped with Devel::Multithreaded::Cover and you can easily
create your own.

The F<gcov2perl> program can be used to convert gcov files to C<Devel::Multithreaded::Cover>
databases.  This allows you to display your C or XS code coverage together
with your Perl coverage, or to use any of the Devel::Multithreaded::Cover reports to display
...
L<Devel::NYTProf>.  Previous versions of Devel::Multithreaded::Cover collected coverage data by
...
variable $DEVEL_MULTITHREADED_COVER_UNSUPPORTED.  Unsupported versions are also untested.  I
...
know if you are just testing Devel::Multithreaded::Cover, only if you are seriously using it to
...
Required if you want to run Devel::Multithreaded::Cover's own tests.
...
By adding C<use Devel::Multithreaded::Cover;> to your mod_perl startup script, you should be
...
Alternatively, add -MDevel::Multithreaded::Cover to the parameters for mod_perl.
In this example, Devel::Multithreaded::Cover will be operating in silent mode.

 PerlSwitches -MDevel::Multithreaded::Cover=-silent,1
...
subroutines containing xx are private, call Devel::Multithreaded::Cover with the option
...
should never happen.  You can tell Devel::Multithreaded::Cover that certain criteria are
...
may be better to record that fact in some formal fashion and stop Devel::Multithreaded::Cover
...
The -silent option is turned on when Devel::Multithreaded::Cover is invoked via
$HARNESS_PERL_SWITCHES or $PERL5OPT.  Devel::Multithreaded::Cover tries to do the right thing
when $MOD_PERL is set.  $DEVEL_MULTITHREADED_COVER_OPTIONS is appended to any options passed
into Devel::Multithreaded::Cover.

Note that when Devel::Multithreaded::Cover is invoked via an environment variable, any modules
...
command line and any code to be covered must appear after Devel::Multithreaded::Cover has been
loaded.  To work around this, Devel::Multithreaded::Cover can also be specified on the command
line.

=head2 Developer variables

When running Devel::Multithreaded::Cover's own test suite, $DEVEL_MULTITHREADED_COVER_DEBUG turns on
debugging information, $DEVEL_MULTITHREADED_COVER_GOLDEN_VERSION overrides Devel::Multithreaded::Cover's
own idea of which golden results it should test against, and
$DEVEL_MULTITHREADED_COVER_NO_COVERAGE runs the tests without collecting coverage.
$DEVEL_MULTITHREADED_COVER_DB_FORMAT may be set to "Sereal", "JSON" or "Storable" to
override the default choice of DB format (Sereal, then JSON if either are
available, otherwise Storable).  $DEVEL_MULTITHREADED_COVER_IO_OPTIONS provides fine-grained
control over the DB format.  For example, setting it to "pretty" when the
format is JSON will store the DB in a readable JSON format.  $DEVEL_MULTITHREADED_COVER_CPUS
...
=item * L<Devel::Multithreaded::Cover::Tutorial>
...
There are things that Devel::Multithreaded::Cover can't cover.
...
Devel::Multithreaded::Cover uses internally.  For example, suppose your program has
this function:
...
Because Devel::Multithreaded::Cover uses Storable internally, the import will succeed
(and the test will fail) under a coverage run.

Modules used by Devel::Multithreaded::Cover while gathering coverage:
...
conjunction with Devel::Multithreaded::Cover.
...
and L<https://rt.cpan.org/Public/Dist/Display.html?Name=Devel-Multithreaded-Cover>
