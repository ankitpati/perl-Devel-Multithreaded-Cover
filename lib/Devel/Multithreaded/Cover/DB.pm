...
package Devel::Multithreaded::Cover::DB;
...
use Devel::Multithreaded::Cover::DB::Structure;
use Devel::Multithreaded::Cover::DB::IO;

use Carp;
use File::Find;
use File::Path;

use Devel::Multithreaded::Cover::Dumper;  # For debugging

use constant CAN_TERM_SIZE => eval { require Term::Size };

my $DB = "cover.14";  # Version of the database

@Devel::Multithreaded::Cover::DB::Criteria =
    (qw( statement branch path condition subroutine pod time ));
@Devel::Multithreaded::Cover::DB::Criteria_short =
    (qw( stmt      bran   path cond      sub        pod time ));
$Devel::Multithreaded::Cover::DB::Ignore_filenames = qr/   # Used by Devel::Multithreaded::Cover
...
sub new {
    my $class = shift;
    my $self  = {
        criteria         => \@Devel::Multithreaded::Cover::DB::Criteria,
        criteria_short   => \@Devel::Multithreaded::Cover::DB::Criteria_short,
        runs             => {},
        collected        => {},
        uncoverable_file => [],
        @_
...
sub read {
    ...
    my $io = Devel::Multithreaded::Cover::DB::IO->new;
...
sub merge_runs {
    ...
    for my $run (sort @runs) {
        # print STDERR "Devel::Multithreaded::Cover: merging run $run <$self->{base}>\n";
        my $r = Devel::Multithreaded::Cover::DB->new(base => $self->{base}, db => $run);
        $self->merge($r);
    }
    ...
    if (keys %{$self->{changed_files}}) {
        my $st = Devel::Multithreaded::Cover::DB::Structure->new(base => $self->{base});
        $st->read_all;
...
sub validate_db {
    ...
    # just warn for now
    print STDERR "Devel::Multithreaded::Cover: $self->{db} is an invalid database\n"
        ...
        for my $file (sort keys %{$frun->{digests}}) {
            ...
            for my $name (sort keys %{$self->{runs}}) {
                ...
                if ($run->{digests}{$file} && $digest &&
                    ...
                    print STDOUT "Devel::Multithreaded::Cover: Deleting old coverage for ",
                                               "changed file $file\n"
                        unless $Devel::Multithreaded::Cover::Silent;
...
sub calculate_summary {
    ...
    Devel::Multithreaded::Cover::Criterion->calculate_percentage($self, $t->{total});
...
sub print_summary {
    ...
    my $width = !$ENV{DEVEL_MULTITHREADED_COVER_TEST_SUITE} && CAN_TERM_SIZE && -t STDOUT
...
sub add_statement {
    ...
    for my $i (0 .. $#$fc) {
        ...
        unless (defined $l) {
            ...
            warn "Devel::Multithreaded::Cover: ignoring extra statement\n";
            return;
        }
        my $n = $line{$l}++;
        no warnings "uninitialized";
...
sub add_branch {
    ...
    for my $i (0 .. $#$fc) {
        my $l = $sc->[$i][0];
        unless (defined $l) {
            warn "Devel::Multithreaded::Cover: ignoring extra branch\n";
            return;
        }
        my $n = $line{$l}++;
        if (my $a = $cc->{$l}[$n]) {
...
sub add_subroutine {
    ...
    for my $i (0 .. $#$fc) {
        ...
        unless (defined $l) {
            # print STDERR "sc ", scalar @$sc, ", fc ", scalar @$fc, "\n";
            # print STDERR "sc ", Dumper($sc), "fc ", Dumper($fc);
            warn "Devel::Multithreaded::Cover: ignoring extra subroutine\n";
            return;
        }
...
sub uncoverable {
    ...
    for my $file (sort keys %$u) {
        ...
        open my $fh, "<", $file or do {
            warn "Devel::Multithreaded::Cover: Can't open $file: $!\n";
            next;
        };
        ...
        for my $crit (keys %$f) {
            ...
            for my $line (keys %$c) {
                if (exists $dl{$line}) {
                    ...
                    warn "Devel::Multithreaded::Cover: Can't find line for uncovered data: " .
                         "$file $crit $line\n";
                    delete $c->{$line};
                }
            }
    ...
    for my $add (@$adds) {
        ...
        if (defined) {
            open my $u, ">>", $uncoverable_file
                or die "Devel::Multithreaded::Cover: Can't open $uncoverable_file: $!\n";
            my $dl = Digest::MD5->new->add($_)->hexdigest;
            print $u "$file $crit $dl $count $type $class $note\n";
        } else {
            warn "Devel::Multithreaded::Cover: Can't find line $line in $file.  ",
                 "Last line is $.\n";
        }
        close $f or die "Devel::Multithreaded::Cover: Can't close $file: $!\n";
    }
}
...
sub uncoverable_comments {
    ...
    open my $fh, "<", $file or do {
        ...
        # warn "Devel::Multithreaded::Cover: Warning: can't open $file: $!\n";
        return;
    };
    my @waiting;
    while (<$fh>) {
...
sub objectify_cover {
    ...
    unless (UNIVERSAL::isa($self->{cover}, "Devel::Multithreaded::Cover::DB::Cover")) {
        bless $self->{cover}, "Devel::Multithreaded::Cover::DB::Cover";
        for my $file (values %{$self->{cover}}) {
            bless $file, "Devel::Multithreaded::Cover::DB::File";
            while (my ($crit, $criterion) = each %$file) {
                next if $crit eq "meta";  # ignore meta data
                my $class = "Devel::Multithreaded::Cover::" . ucfirst lc $crit;
                bless $criterion, "Devel::Multithreaded::Cover::DB::Criterion";
        ...
        for my $r (keys %{$self->{runs}}) {
            if (defined $self->{runs}{$r}) {
                bless $self->{runs}{$r}, "Devel::Multithreaded::Cover::DB::Run";
            } else {
                delete $self->{runs}{$r};  # DEVEL_MULTITHREADED_COVER_SELF
            }
        }
    }

    unless (exists &Devel::Multithreaded::Cover::DB::Base::items) {
        *Devel::Multithreaded::Cover::DB::Base::items = sub {
            my $self = shift;
            keys %$self
        };

        *Devel::Multithreaded::Cover::DB::Base::values = sub {
            my $self = shift;
            values %$self
        };

        *Devel::Multithreaded::Cover::DB::Base::get = sub {
            my $self = shift;
            my ($get) = @_;
            $self->{$get}
        };
        ...
        my $base = "Devel::Multithreaded::Cover::DB::Base";
        while (my ($class, $functions) = each %$classes) {
            my $c = "Devel::Multithreaded::Cover::DB::$class";
            no strict "refs";
            @{"${c}::ISA"} = $base;
        ...
        *Devel::Multithreaded::Cover::DB::File::DESTROY = sub {};
        unless (exists &Devel::Multithreaded::Cover::DB::File::AUTOLOAD) {
            *Devel::Multithreaded::Cover::DB::File::AUTOLOAD = sub {
                # Work around a change in bleadperl from 12251 to 14899
                my $func = $Devel::Multithreaded::Cover::DB::AUTOLOAD || $::AUTOLOAD;
...
sub cover {
    ...
    my $st = $self->{_structure}
        // Devel::Multithreaded::Cover::DB::Structure->new(base => $self->{base})->read_all;
    ...
    for my $run (@runs) {
        ...
        my $r = $self->{runs}{$run};
        next unless $r->{collected};  # DEVEL_MULTITHREADED_COVER_SELF
        ...
        while (my ($file, $f) = each %$count) {
            ...
            unless ($digest) {
                print STDERR "Devel::Multithreaded::Cover: Can't find digest for $file\n"
                    unless $Devel::Multithreaded::Cover::Silent ||
                           $file =~ $Devel::Multithreaded::Cover::DB::Ignore_filenames ||
                           ($Devel::Multithreaded::Cover::Self_cover &&
                            $file =~ q|/Devel/Multithreaded/Cover[./]|);
                next;
            }
            # print STDERR "File: $file\n";
            print STDERR "Devel::Multithreaded::Cover: merging data for $file ",
                         "into $digests{$digest}\n"
            ...
            while (my ($criterion, $fc) = each %$f) {
                ...
                unless ($sc) {
                    print STDERR "Devel::Multithreaded::Cover: Warning: can't locate ",
...
package Devel::Multithreaded::Cover::DB::Run;
...
Devel::Multithreaded::Cover::DB - Code coverage metrics for Perl

=head1 SYNOPSIS

 use Devel::Multithreaded::Cover::DB;

 my $db = Devel::Multithreaded::Cover::DB->new(db => "my_coverage_db");
...
Returns a Devel::Multithreaded::Cover::DB::Cover object.  From here all the coverage
...
Devel::Multithreaded::Cover::DB::Base (the base class for all Devel::Multithreaded::Cover::DB::* classes):

=over

=item * Devel::Multithreaded::Cover::DB::Base->values

Aliased to Devel::Multithreaded::Cover::DB::Cover->files, Devel::Multithreaded::Cover::DB::File->criteria,
Devel::Multithreaded::Cover::DB::Criterion->locations, and Devel::Multithreaded::Cover::DB::Location->data

=item * Devel::Multithreaded::Cover::DB::Base->get

Aliased to Devel::Multithreaded::Cover::DB::Cover->file, Devel::Multithreaded::Cover::DB::File->criterion,
Devel::Multithreaded::Cover::DB::Criterion->location, and Devel::Multithreaded::Cover::DB::Location->datum
...
=head1 SEE ALSO
 ...
 Devel::Multithreaded::Cover::DB::Base
 Devel::Multithreaded::Cover::DB::Cover
 Devel::Multithreaded::Cover::DB::File
 Devel::Multithreaded::Cover::DB::Criterion
 Devel::Multithreaded::Cover::DB::Location
