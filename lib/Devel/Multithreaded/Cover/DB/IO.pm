...
package Devel::Multithreaded::Cover::DB::IO;
...
sub new {
    ...
    my $format = $ENV{DEVEL_MULTITHREADED_COVER_DB_FORMAT} || $Format;
    ($format) = $format =~ /(.*)/;  # die tainting
    die "Devel::Multithreaded::Cover: Unrecognised DB format: $format"
        unless $format =~ /^(?:Storable|JSON|Sereal)$/;

    $class .= "::$format";
    eval "use $class; 1" or die "Devel::Multithreaded::Cover: $@";

    $class->new(options => $ENV{DEVEL_MULTITHREADED_COVER_IO_OPTIONS} || "", @_)
}
...
Devel::Multithreaded::Cover::DB::IO - IO routines for Devel::Multithreaded::Cover::DB

=head1 SYNOPSIS

 use Devel::Multithreaded::Cover::DB::IO;

 my $io = Devel::Multithreaded::Cover::DB::IO->new(format => "JSON");
 my $data = $io->read($file);
 $io->write($data, $file);

=head1 DESCRIPTION

This module provides IO routines for Devel::Multithreaded::Cover::DB.
