...
package Devel::Multithreaded::Cover::Op;
...
use Devel::Multithreaded::Cover qw( -ignore blib -ignore \\wB\\w );
...
sub import {
    ...
    add_callback(sub {
        ...
        my $key = Devel::Multithreaded::Cover::get_key($op);
        # print Dumper Devel::Multithreaded::Cover::coverage unless $d++;
        if ($h->{seq}) {
            my ($s, $b, $c) =
              map Devel::Multithreaded::Cover::coverage($final ? $final-- : 0)->{$_}{$key},
...
Devel::Multithreaded::Cover::Op - B::Concise with coverage data

=head1 SYNOPSIS

 perl -Mblib -MDevel::Multithreaded::Cover::Op prog [options]

=head1 DESCRIPTION

This module works as if calling B::Concise but also outputs coverage
information.  Its primary purpose is to aid in the development of Devel::Multithreaded::Cover.
