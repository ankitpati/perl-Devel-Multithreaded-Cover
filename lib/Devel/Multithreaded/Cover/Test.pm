...
package Devel::Multithreaded::Cover::Test;
...
sub new {
    ...
    my $self = bless {
        ...
        changes          => [],
        test_parameters  => [],
        debug            => $ENV{DEVEL_MULTITHREADED_COVER_DEBUG} || 0,
        differences      => $differences,
        no_coverage      => $ENV{DEVEL_MULTITHREADED_COVER_NO_COVERAGE} || 0,
        delay_after_run  => 0,
        %params
    }, $class;
...
sub get_params {
    ...
    $self->{test_parameters}  = "$self->{select}"
                              . " -ignore blib Devel/Multithreaded/Cover @{$self->{ignore}}"
...
sub perl {
    ...
    join " ",
         map shell_quote($_),
             $Devel::Multithreaded::Cover::Inc::Perl,
...
sub test_command {
    ...
    unless ($self->{no_coverage}) {
        $c .= " "
           .  shell_quote "-MDevel::Multithreaded::Cover=" .
...
sub cover_gold {
    ...
    my $v = exists $ENV{DEVEL_MULTITHREADED_COVER_GOLDEN_VERSION}
        ? $ENV{DEVEL_MULTITHREADED_COVER_GOLDEN_VERSION}
...
sub run_test {
    ...
    $ENV{DEVEL_MULTITHREADED_COVER_TEST_SUITE} = 1;
...
sub run_cover {
    ...
    my $change_line = sub {
        ...
        while (1) {
            ...
            redo if /^Devel::Multithreaded::Cover: merging run/;
            ...
            s/-+/-/;
            # s/.* Devel-Multithreaded-Cover - \d+ \. \d+ \/*(\S+)\s*/$1/x;
            s/^ \.\.\. .* - \d+ \. \d+ \/*(\S+)\s*/$1/x;
            s/.* Devel \/ Cover \/*(\S+)\s*/$1/x;
            s/^(Devel::Multithreaded::Cover: merging run).*/$1/;
            s/^(Run: ).*/$1/;
            s/^(OS: ).*/$1/;
            s/^(Perl version: ).*/$1/;
            s/^(Start: ).*/$1/;
    ...
    # use Devel::Multithreaded::Cover::Dumper; print STDERR "--->", Dumper $self->{changes};
...
sub create_gold {
    ...
    # use Devel::Multithreaded::Cover::Dumper; print STDERR Dumper $self;
    if ($self->{skip}) {
    ...
    open T, "$cover_com 2>&1 |" or die "Cannot run $cover_com: $!";
    while (my $l = <T>) {
        next if $l =~ /^Devel::Multithreaded::Cover: merging run/;
        $l =~ s/^($_: ).*$/$1.../
...
Devel::Multithreaded::Cover::Test - Internal module for testing
...
=head2 new
  ...
  my $test = Devel::Multithreaded::Cover::Test->new($test, criteria => $string)
...
=item Devel::Multithreaded::Cover -M option (if applicable)
