...
package Devel::Multithreaded::Cover::DB::Criterion;
use Devel::Multithreaded::Cover::Truth_Table;
use strict;
use warnings;
...
sub truth_table {
	...
	foreach my $c (@$c) {
		...
		my $t = {
			tt   => Devel::Multithreaded::Cover::Truth_Table->new_primitive($op, @hit),
			cvg  => $c->[1],
...
package Devel::Multithreaded::Cover::Truth_Table::Row;
use warnings;
use strict;
...
sub new {
	...
	my @args = @_;
        # use Devel::Multithreaded::Cover::Dumper; print Dumper \@args;
	return bless {
		inputs    => $args[0],
		result    => $args[1],
		covered   => $args[2],
...
package Devel::Multithreaded::Cover::Truth_Table;
use warnings;
use strict;
# VERSION
...
#
sub and_tt {
	return(Devel::Multithreaded::Cover::Truth_Table::Row->new([0, 'X'], 0, shift),
           Devel::Multithreaded::Cover::Truth_Table::Row->new([1,  0 ], 0, shift),
           Devel::Multithreaded::Cover::Truth_Table::Row->new([1,  1 ], 1, shift));
}
sub or_tt {
	return(Devel::Multithreaded::Cover::Truth_Table::Row->new([0,  0 ], 0, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([0,  1 ], 1, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([1, 'X'], 1, shift));
}
sub xor_tt {
	return(Devel::Multithreaded::Cover::Truth_Table::Row->new([0, 0], 0, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([0, 1], 1, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([1, 0], 1, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([1, 1], 0, shift));
}
sub boolean_tt {
	return(Devel::Multithreaded::Cover::Truth_Table::Row->new([0], 0, shift),
	       Devel::Multithreaded::Cover::Truth_Table::Row->new([1], 1, shift));
}
...
sub right_merge {
	...
	foreach my $row1 (@$tt1) {
		if ($row1->rightcol eq 'X') {
			push(@tt, Devel::Multithreaded::Cover::Truth_Table::Row->new([$row1->leftelems, @dontcare],
			...
			foreach my $row2 (@{$merge[$row1->rightcol]}) {
				push(@tt, Devel::Multithreaded::Cover::Truth_Table::Row->new([$row1->leftelems, $row2->inputs],
...
sub left_merge {
	...
	foreach my $row1 (@$tt1) {
		...
		foreach my $row2 (@{$merge[$row1->leftcol]}) {
			# expand value from tt1 with rows from tt2 that result in
			# that value
			push(@tt, Devel::Multithreaded::Cover::Truth_Table::Row->new([$row2->inputs, $row1->rightelems],
				$row1->result,
...
Devel::Multithreaded::Cover::Truth_Table - Truth tables for coverage objects.
...
=head1 SYNOPSIS
  ...
  # $a || $b
  my $or_tt  = Devel::Multithreaded::Cover::Truth_Table->new_primitive('or_3', 0, 1, 1);

  # $c && $d
  my $and_tt = Devel::Multithreaded::Cover::Truth_Table->new_primitive('and_3', 1, 0, 1);
...
are the subclasses of Devel::Multithreaded::Cover::DB::Condition. C<$op> is one of the
following:
