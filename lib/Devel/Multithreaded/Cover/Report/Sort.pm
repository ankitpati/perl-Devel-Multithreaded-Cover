...
package Devel::Multithreaded::Cover::Report::Sort;
...
sub print_sort {
    ...
    # use Devel::Multithreaded::Cover::Dumper; print Dumper [$db->runs];
    for my $r (sort {$a->{start} <=> $b->{start}} $db->runs) {
        ...
        # use Devel::Multithreaded::Cover::Dumper; print Dumper $r;

        @{$runs{$r->run}}{"vec", "size"} = ("", 0);
        my $run = $runs{$r->run};
        # use Devel::Multithreaded::Cover::Dumper; print Dumper $run;
        my $vec = $r->vec;
...
Devel::Multithreaded::Cover::Report::Sort - backend for Devel::Multithreaded::Cover
